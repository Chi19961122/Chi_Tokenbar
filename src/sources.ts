// T-916 供應商多選: the unified usage/quota source selection. One multi-select
// of five sources replaces the old 3-way `providers` filter plus the two
// `tool_*` checkboxes. These are the pure derivations the island, panel,
// analytics cache key and settings UI all read from — kept here so the display
// matrix (which quota providers → which island layout) stays unit-testable.

import type { ProviderFilter } from "./types";

export type SourceId = "claude" | "codex" | "opencode" | "gemini" | "grok";

/** Canonical order for the chip row and the fresh-install default. */
export const ALL_SOURCES: SourceId[] = ["claude", "codex", "opencode", "gemini", "grok"];
 ...
 *
 * Only claude/codex drive the island — opencode/gemini/grok are usage-only.
 */
