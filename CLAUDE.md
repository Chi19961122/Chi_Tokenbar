...
Windows 常駐的 AI coding 額度 runway 監控器(Tauri 2 + vanilla TS)。監控 Claude Code 與 Codex 的 5h/週限制,island pill + 系統匣(靜態 app logo,額度數字在 tooltip)+ 展開面板 + 用量分析。
...
## 常用指令
- 測試(後端):`cargo test --manifest-path src-tauri\Cargo.toml`(PATH 需先加 `%USERPROFILE%\.cargo\bin`)
- 測試(前端):`npm test`(vitest + jsdom,跑 `src/**/*.test.ts`;純排版不測,但**決策邏輯**要抽成函式再測 —— 如 `island::islandIntent`)
- 開發:`npm run tauri dev`(設 `TOKENBAR_DEBUG=1` 可在 stderr 看 `[tb]` 每輪數值)
- 打包:`npm run build:release` → 安裝檔集中複製到 repo 外的 `..\Atoll-release\`(NSIS/MSI/免安裝 exe;原始產物仍在 `src-tauri\target\release\bundle\`)
