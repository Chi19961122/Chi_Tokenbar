...
- F-13 [func] token/花費切換在多個頁籤沒反應、模型/工具切換部分頁籤也是。**根因**：非本輪回歸——後端每筆記錄都有算成本（Claude+Codex），但只累加到每日桶；每時/各模型/各工具從來沒有成本資料可畫，佔比以外的頁籤 group 切換也本無視覺差異。**修**：後端補 `hourlyCost`/`byModelCost`/`byAgentCost` 三個維度（與 token 桶同鍵同分桶）；前端花費模式真的畫成本（每時長條、佔比條 `$·%` 標籤、兩種模式都加逐條 tooltip）；切換鈕收斂到有作用的頁籤——metric 只在總覽/佔比/每時、group 只在佔比、統計頁只剩範圍。executor 實作＋verifier CONFIRMED（前端 88、後端 159 測試綠）。→ `c43c543`。
