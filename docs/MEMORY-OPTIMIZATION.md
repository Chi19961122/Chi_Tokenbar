...
> 狀態：**規劃檔 v2.3** — **階段 0 instrumentation + 1A 已實作（本機）**；1B／2／3 仍待做  
> 日期：2026-07-19（v2.3 同日；0/1A 實作同日）  
...
> **下一步：** 1B scan coordinator（可選 month 衍生）→ 2 parser → 必要時 SQLite。
...
| **1B. Coordinator** | exclusion + coalesce + TTL + queue；非裸 mutex | 消並行尖峰與重複 key 重掃 | 中 | **已確認** 需求 |
